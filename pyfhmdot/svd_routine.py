...
def mult_deg_MV(array_U, array_S, cut, array_V, deg, subnewsize, dict_left, dict_right):
...
def mult_deg_UM(array_U, array_S, cut, array_V, deg, subnewsize, dict_left, dict_right):
    i_Nb = len(deg)
    for i in range(i_Nb):
        Dsi = cut.pop()
        if Dsi > 0:
